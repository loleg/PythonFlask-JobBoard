...
@app.route('/employer/<employer_id>/post', methods=('GET', 'POST'))
def newjob(employer_id):
    if request.method == 'POST':
        ...
        date = datetime.datetime.now().strftime("%m/%d/%Y")
        execute_sql(
            'INSERT INTO job (title, description, salary, date, employer_id) VALUES ' +
            '(?, ?, ?, ?, ?)',
            (title, description, salary, date, employer_id), commit=True)
        return redirect(url_for('employer', employer_id=employer_id))
